# 👋 Olá, eu sou o Ray Labres  

💻 **Analista de Segurança da Informação & Engenharia de Dados**  
🔍 Focado em **Threat Intelligence, PLD/AML, antifraude, automações e IA aplicada à segurança**  
🚀 Apaixonado por tecnologia, dados e arquitetura de sistemas escaláveis  

---

## 🧠 Sobre mim
Sou estudante de Ciência da Computação e atuo como Analista de Segurança da Informação, com experiência em:  
- Desenvolvimento de **pipelines de dados e regras antifraude**
- Análise comportamental e detecção de anomalias  
- Automação com **Python, SQL e Machine Learning**
- Projetos de **IA, OCR e visão computacional em iniciação científica**

---

## 🔥 GitHub Streak

<p align="center">
  <img src="https://streak-stats.demolab.com?user=raylabres&theme=tokyonight&hide_border=true&locale=pt_BR" />
</p>

---

## 🧰 Tecnologias & Ferramentas

### 🖥️ Linguagens e Frameworks
<p>
  <img src="https://img.shields.io/badge/Python-000?style=for-the-badge&logo=python"/>
  <img src="https://img.shields.io/badge/Flask-000?style=for-the-badge&logo=flask"/>
  <img src="https://img.shields.io/badge/FastAPI-000?style=for-the-badge&logo=fastapi"/>
  <img src="https://img.shields.io/badge/Django-000?style=for-the-badge&logo=django"/>
  <img src="https://img.shields.io/badge/SQL-000?style=for-the-badge&logo=mysql"/>
  <img src="https://img.shields.io/badge/HTML5-000?style=for-the-badge&logo=html5"/>
  <img src="https://img.shields.io/badge/CSS3-000?style=for-the-badge&logo=css3"/>
</p>

### 🗄️ Banco de Dados
<p>
  <img src="https://img.shields.io/badge/PostgreSQL-000?style=for-the-badge&logo=postgresql"/>
  <img src="https://img.shields.io/badge/MySQL-000?style=for-the-badge&logo=mysql"/>
  <img src="https://img.shields.io/badge/SQL_Server-000?style=for-the-badge&logo=microsoftsqlserver"/>
</p>

### 🔐 Segurança & Dados
- Threat Intelligence  
- PLD/AML e antifraude  
- Machine Learning aplicado à detecção de risco  
- Web Scraping + OCR  
- Automação e pipelines de dados  

---

## 🚀 Projetos em Destaque

### 🔐 Fraud & AML Behavioral Engine  
Sistema de análise comportamental para scoring de risco transacional, reduzindo falsos positivos e custos operacionais.  
**Stack:** Python, SQL, Machine Learning, Data Pipelines  

### 🤖 AI OCR Web Scraping Research  
Projeto de iniciação científica para extração automática de texto em imagens de páginas web usando OCR e visão computacional.  

### 🎥 Multi-Camera PPE Detection System  
Sistema de detecção de EPI em tempo real com YOLOv5 e backend Flask para monitoramento industrial.  

---

## 🌐 Contato

<p>
  <a href="https://www.linkedin.com/in/ray-labres">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin"/>
  </a>
  <a href="mailto:raylabres@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-333333?style=for-the-badge&logo=gmail"/>
  </a>
</p>
